{include file="orderforms/cloudstamp_cart/common.tpl"}

<script type="text/javascript" src="templates/orderforms/{$carttpl}/js/jquery.steps.min.js"></script>
<link rel="stylesheet" type="text/css" href="templates/orderforms/{$carttpl}/css/jquery.steps.css" />
    ...
    <div class="row">
        ...
        <div class="pull-md-right col-md-9">
            ...
            <div class="header-lined">
                <h1>{$LANG.orderconfigureCS}</h1>
            </div>

        </div>

        <!--
		We should remove this to not show the left side options in the cart
            ...
            <form id="frmConfigureProduct" onsubmit="catchEnter(event);">
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <p><i>{$LANG.orderForm.configureDesiredOptions}</i></p>
						
						<br />

                        <div class="product-info">
                            <p class="product-title"><strong>{$productinfo.name}</strong></p>
                        ...
                        {if $pricing.type eq "recurring"}
                            ...
                            <div class="field-container">
                                <div class="form-group">
                                    {if $pricing.quarterly || $pricing.semiannually || $pricing.annually || $pricing.biennially || $pricing.triennially}
									...
									{else}
										<label for="inputBillingcycle">{$LANG.orderbillingcycle}</label>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    {if $configoption.qtymaximum}
                                                        ...
                                                        <script>
															...
															{if preg_match('/Solid State Hard Drive/i',$configoption.optionname)}
															ssd = jQuery("#inputConfigOption{$configoption.id}");
															ssd.ionRangeSlider({
															{elseif preg_match('/Hard Disk Drive/i',$configoption.optionname)}
															hdd = jQuery("#inputConfigOption{$configoption.id}");
															hdd.ionRangeSlider({
															{else}
															...
															{/if}
																...
																onFinish: function() {
																	{if preg_match('/Solid State Hard Drive/i',$configoption.optionname)}
																	updateFromTo('ssd','inputConfigOption{$configoption.id}');
																	{elseif preg_match('/Hard Disk Drive/i',$configoption.optionname)}
																	updateFromTo('hdd','inputConfigOption{$configoption.id}');
																	{/if}
																}
                                                            });
															{if preg_match('/Solid State Hard Drive/i',$configoption.optionname)}
															var ssdSlider = $("#inputConfigOption{$configoption.id}").data("ionRangeSlider");
															{elseif preg_match('/Hard Disk Drive/i',$configoption.optionname)}
															var hddSlider = $("#inputConfigOption{$configoption.id}").data("ionRangeSlider");
															{/if}
                                                        </script>
                                                    {else}
                                                        <div>
...
<script>
function updateFromTo(type, fieldId) {
	console.log("type: "+type);
	console.log("fieldId: "+fieldId);
    if (type == 'ssd') {
        ssdVal = $("input#"+fieldId).val();
		console.log("ssdVal: ".ssdVal);
        hddSlider.update({
            min: 0,
            max: 24 - ssdVal
        });
    ...
    if (type == 'hdd') {
        hddVal = $("input#"+fieldId).val();
		console.log("hddVal: ".hddVal);
        ssdSlider.update({
            min: 0,
            max: 24 - hddVal
        });
	...
	wizard.children("div").steps({
		...
		{
			alert("I think we will submit now");
		}
	});
